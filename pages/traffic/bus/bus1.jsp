...
<head lang="en">
 ...
 <script type="text/javascript" src="http://api.map.baidu.com/api?v=1.5&ak=ezofnKG8X1gQ29UuOUjQlo1Q"></script>
    ...
    <style type="text/css">
		#setmap{height:0px;width:0px;}
	 	...
	 	#aroundPoint>ul{background:#fff;padding:10px;}
		#aroundPoint>li{height:60px;border-bottom:1px solid #ccc;background:#fff;line-height:60px;}
		#aroundPoint>li>span{float:left;display:inline-block;height:100%;line-height:50px;}
		#aroundPoint>li>span>img{width:20px;vertical-align: middle;margin-left:10px;margin-top:5px;}
		#aroundPoint>li>p{float:left;margin-left:10px;}
		#aroundPoint>li>.sp{float:right;margin-right:20px;color:#999;line-height:60px;}
		#aroundPoint>li>.sp>i{margin-left:10px;}
		#aroundPoint>li>.sp>i>img{width:22px;vertical-align: middle;margin-right:10px;}  
  ...
  <!--   <a class="topIcon" href="#userDetail"><i class="fa fa-user"></i></a> -->
    ...
    <div class="brandBox">
    	...
    	<a class="putBox"  onclick="showAllLines();">
    		...
    		<span class="center">点击查看城际巴士全部线路</span>
    		<span class="right" ><img class="img" src="${ctx }/resource/image/main/more.png"></span>
    	</a>
      
        <div class="exchangeInput">
        	<div style="float:right;height:100%;line-height:100px;display:none;">
        		<img src="${ctx }/resource/image/main/u72.png" style="width:40px;height:40px;vertical-align: middle;">
        	</div>
        	<div class="inputContainer">
	        	<div>
	        		...
	        		<input type="text" name="starName" id="startSite" readonly="readonly" placeholder="请选择你要从哪儿出发" form="busForm"  />
	        		<a class="taged"  id="selectstar"><img src="${ctx }/resource/image/main/u46.png" style="width:22px;transform: rotate(-90deg);vertical-align: middle;"></a>
	        	</div>
	        	<div class="">
	        		<img class="img"  src="${ctx }/resource/image/main/co-red1.png" />
	        		<!-- <span>目的地</span> -->
	        		<input type="text" name="endName" id="endSite"  readonly="readonly"  placeholder="请选择你要去哪儿" form="busForm" />
	        		<a class="taged"  id="selectend" ><img src="${ctx }/resource/image/main/u46.png" style="width:22px;transform: rotate(-90deg);vertical-align: middle;"></a>
	        	</div>
...
<!-- 	        <input type="hidden"  id="starLat" name="starLat" value=""  form="busForm"  /> -->
        	...
        	<form id="busForm" action="${ctx }/bus/getsite.do" method="post">
	    	 <a class="btn blue" onclick="selectLine()"><span><img src="${ctx }/resource/image/main/u94.png" style="width:20px;margin-right:10px;vertical-align: middle;"></span>查询线路</a>
    	</form>
   ...
   <div class="aroundPoint">
        	...
        	<div>
        		<ul id="aroundPoint">
					...
					<%-- <li style="height:60px;border-bottom:1px solid #ccc;">
						<span style="float:left;display:inline-block;height:100%;line-height:50px;"><img src="${ctx }/resource/image/main/u59.png" style="width:20px;vertical-align: middle;"></span>
						<p style="float:left;margin:5px 0 5px 5px">南塔</p>
						<p style="float:left;margin:15px 0 15px -25px;color:#999;">
							<span>4号线浑(下行),4号线浑(上行)</span>
							<i style="margin-left:10px;"><img src="${ctx }/resource/image/main/u70.png" style="width:22px;vertical-align: middle;"></i>
							<span><100m</span>
						</p>
					</li>
					<li style="height:60px;border-bottom:1px solid #ccc;">
...
*/			
			//////////////////	
									 ...
									 $.ajax({url:"${ctx}/bus/initloc.do?json",async: false,
											...
											success:function(result){
												...
												if (typeof (result) != 'undefined' && result != null){
													...
													var slat=latinit;
													var slon=lnginit;		
													total=resSite.length;
													for(var i=0;i<resSite.length;i++){
														forDistance(slon,slat,resSite[i].b_lon,resSite[i].b_lat);
														//upload+='<span class="taged" id="staNear'+i+'" onclick="selectSiteLines(this,'+resSite[i].sta_id+','+resSite[i].b_lon+','+resSite[i].b_lat+')" >'+resSite[i].sta_name+'</span>';
														
														upload+='<li onclick="selectSiteLines('+i+')" ><span><img src="${ctx }/resource/image/main/ubus.png"></span>';
														upload+='<p id="nearName'+i+'">'+resSite[i].sta_name+'</p>';
														upload+='<p class="sp"><i><img src="${ctx }/resource/image/main/u70.png"></i>';
														upload+='<input type="hidden" id="nearLat'+i+'" value="'+resSite[i].b_lat+'"></input>';
														upload+='<span id="nearDistance'+i+'" >暂无</span></p></li>';
				...
				function forDistance(slon,slat,lon,lat){								 
				 	var  siteMy1= new BMap.Point(slon,slat);
					...
					var walking = new BMap.WalkingRoute(map, {renderOptions:{map: map, autoViewport: true}});			
					 ...
					 walking.setSearchCompleteCallback(function(result){
							...
							var ssite=result.getEnd().point.lat;
							var num1=ssite.toFixed(4);	
							//alert("返回="+num1);
							range=plan.getDistance(true);							
							
							for(var i=0;i<total;i++){
								var tempLat="#nearLat"+i;							
								var tempNum=$(tempLat).val();
								//alert("tempNum"+tempNum);
								var num2=parseFloat(tempNum).toFixed(4);
								//alert("原来"+num2);
							
								if(num1 == num2){
									var tempD="#nearDistance"+i;	
									$(tempD).html(range);	
						 			break;
					 			}
							}
							
							//var temp="#nearDistance"+index;	
							//index++;				
							//alert(range);
							//$(temp).html(range);								
					});								
	 ...
	 //根据附近站点实现线路展示
	 function selectSiteLines(index){
		/* 	if(latinit==null||latinit==''){
 				return;			
 			} */	
 			//自己得到range
 			//alert("进入方法")
 			var sitename=$("#nearName"+index).html();
 			var range=$("#nearDistance"+index).html();
 			//alert("sitename"+sitename);
 			//alert("range"+range);
		window.location.href="${ctx}/bus/siteline.do?sitename="+sitename+"&range="+range; 
	}
	...
	//选择一个站点后，去查与此站点相关的站点
	 function qq(direction,sitename){
		 $.ajax({
			 	url:"${ctx}/bus/getposlist2.do?json",
				async: false,
				data:{sitename:sitename,direction:direction}, 
				type: "POST",
				dataType:'json',
				error:function(){},
				success:function(result){
