				...
				<a>
					<ul>
						...
						<li class="secondRow">
							...
							<span><small style="font-size:14px !important;">全程票价：<i class="fa fa-jpy"></i>{{busDetail.price}}</small></span>
						</li>
						<li class="thirdRow">						
							<span>首车时间：{{busDetail.start_time}}</span><span>末车时间：{{busDetail.end_time}}</span>
							<span>车辆间隔：					
								<small>{{busDetail.interval_time}}</small>
							</span><span>运行时长：约{{busDetail.drive_times}}分钟</span>
						</li>
 						<!-- <li class="thirdRow">
		...
		$.ajax({url:"${ctx}/bus/getLines.do?json",async: false,
			...
			success:function(result){
			   ...
			   app.controller('myCtrl', function($scope) {
				   var temp = result.lineInfo[0];
				   if(temp.interval_time == null){
					   temp.interval_time = "暂无";
				   }
				   $scope.busDetail=temp;
