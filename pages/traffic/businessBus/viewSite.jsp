...
<div class="box stopPoint">
   ...
   <%--  <div class="topColor">
    ...
    <div class="mainBox bottom0">
      ...
      <!-- <span class="taged tip bgred"  id="r-result" >请输入或者在地图上选择下车地点！！<input type="text" id="suggestId" size="20" value="请输入地点:" style="width:auto;" />
       	<div id="searchResultPanel" style="border:1px solid #C0C0C0;zwidth:auto;height:auto; display:none;"></div>
       	</span>
       	<span class="taged tip bottom"  id="carprice" >在红线区域内收费为25元！<i class="fa fa-times"></i></span> -->
       <a class="btn blue" href="javascript:window.close()">关闭</a>
    </div>
    <%-- 	<form id="businessCarForm" action="${ctx }/busniessbus/busniessorder.do" method="post">
       ...
       <input type="hidden" id="startLat" name="startLat" value="${startLat }" >
       <input type="hidden" id="startLon" name="startLon" value="${startLon }" >
...
var endLat='${endLat}';
var endLon='${endLon}';
var cityName='${endSite}';
	/* $(function(){
		var url="http://api.map.baidu.com/geocoder/v2/?ak=44lCkR0Vgtga2pFmY82pE2y9&callback=renderReverse&location="+endLat+","+endLon+"&output=json&pois=1";
		$.ajax({
			url:url,
			type:"GET",
			dataType:"json",
	  ...
	  var tempMarker = new BMap.Marker(new BMap.Point(endLon, endLat));
	  
	  map.addOverlay(tempMarker);    //添加标注		            
	  //自己修改
	 var tempLabel = new BMap.Label("下车位置",{offset:new BMap.Size(20,-10)});
	 ...
	 map.centerAndZoom(cityName, 11);
		...
		/* $(function(){
			$.ajax({
				url:"${ctx}/busniessbus/getLines.do?json",
				async: false,
		...
		function showLines(result){
			...
			map.addEventListener("click",function(e){
				...
				map.removeOverlay(markerChoose);				
				map.addOverlay(marker);
				var label = new BMap.Label("下车位置",{offset:new BMap.Size(20,-10)});
