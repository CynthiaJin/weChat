...
(function(t_util){
    ...
    t_util.BASE_PATH = localhostPaht+util.PROJECT_NAME;
	...
	t_util.loadMulit = function(parentDiv,targetDiv,targetContent,defindedprovId,targetUrl,changeFun,provId){
		if(!targetUrl){
		...
		$.ajax({
			url:util.warpRandom(t_util.PROJECT_NAME+targetUrl),
			type: "POST",
			dataType:'json',
			error:function(){
				top.jAlert("服务器异常");
